...
export default function CallSub() {
  ...
  useEffect(() => {
    const loadProviders = async () => {
      try {
        ...
        const rows = Array.isArray(res.data?.data) && res.data.data.length > 0 ? res.data.data : [{ key: 'airtel', label: 'Airtel', description: 'Airtel call bundles' }];
        setProviders(rows);
        ...
        void error;
        setProviders([{ key: 'airtel', label: 'Airtel', description: 'Airtel call bundles' }]);
      }
    };
    void loadProviders();
  }, [activeProvider]);
  ...
  const active = useMemo(
    () => providers.find((provider) => provider.key === activeProvider) || providers[0] || { key: 'airtel', label: 'Airtel', description: 'Airtel call bundles' },
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Choose a network and buy call subscription bundles from one shared module.</p>
      </div>
      ...
      {ActiveProviderComponent ? (
        ...
        <div className="bg-white border border-dashed border-gray-300 rounded-2xl p-6 text-sm text-gray-500">
          {active.label} will appear here once its call sub component is added.
        </div>
      )}
    </div>
  );
