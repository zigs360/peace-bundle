...
describe('Virtual Account secure view', () => {
  ...
  it('does not display virtual accounts from unapproved providers', async () => {
    const user = await User.create({
      name: 'Local VA User',
      email: `local_va_${Date.now()}@test.com`,
      ...
      virtual_account_name: 'Local VA User',
      metadata: { va_provider: 'local' },
    });

    const token = jwt.sign({ id: user.id }, process.env.JWT_SECRET);
    const res = await request(app).get('/api/users/virtual-account').set('Authorization', `Bearer ${token}`);
    ...
    expect(reveal.body.success).toBe(false);
  });
  ...
  it('returns masked account number and reveals full number on reveal endpoint', async () => {
    const user = await User.create({
      ...
      metadata: { va_provider: 'payvessel', payvessel_tracking_reference: 'PV-REF-1' },
    });
  ...
  it('accepts audit events with valid actions', async () => {
    const user = await User.create({
      ...
      metadata: { va_provider: 'payvessel', payvessel_tracking_reference: 'PV-REF-2' },
    });
